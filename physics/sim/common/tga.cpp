...
char *getRGBA (FILE *s, int size)
{
    ...
    rgba = (char*)malloc (size * 4);

    if (rgba == NULL)
        return 0;

    bread = fread (rgba, sizeof (char), size * 4, s);
...
char *getRGB (FILE *s, int size)
{
    ...
    rgb = (char*)malloc (size * 3);

    if (rgb == NULL)
        return 0;

    bread = fread (rgb, sizeof (char), size * 3, s);
...
=============
*/
char *getGray (FILE *s, int size)
{
    char *grayData;
    int bread;

    grayData = (char*)malloc (size);
    ...
    bread = fread (grayData, sizeof (char), size, s);
