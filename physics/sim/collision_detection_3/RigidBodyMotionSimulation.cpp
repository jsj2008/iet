...
//		body->LoadModel("../media/models/torus.obj");
	...
	/*
	m_Triangle->SetPoint( Point3(1.0f,3.0f,0.0f), 0 );
	m_Triangle->SetPoint( Point3(2.0f,1.0f,0.0f), 1 );
	m_Triangle->SetPoint( Point3(2.0f,2.0f,0.0f), 2 );
	*/

	m_Triangle->SetPoint( Point3(0.3f,0.3f,0.0f), 0 );
	m_Triangle->SetPoint( Point3(1.3f,1.3f,0.0f), 1 );
	m_Triangle->SetPoint( Point3(2.3f,0.3f,0.0f), 2 );
	m_Triangle->CreateEdges();
...
void RigidBodyMotionSimulation::VOnRender()
{
	...
	glPushMatrix();
		...
		glVertex3f(m_Particle->GetPos().GetX(),
					m_Particle->GetPos().GetY(),
					m_Particle->GetPos().GetZ());
		Point3 closestFeaturePos = m_Triangle->GetClosestFeaturePos();
...
//
	bool found = false;
	Point3 closestPoint = m_Triangle->GetPoint(0);
	...
	for(int i = 0 ; i < 3 ; i++)
	{
		if(m_VoronoiChecker->PointAndPoint(i, m_Particle->GetPos(), (*m_Triangle)))
		{
			Point3 point = m_Triangle->GetPoint(i);
	...
	if(!found)
	{
		for(int i = 0 ; i < 3 ; i++)
		{
			if(m_VoronoiChecker->PointAndEdge(i, m_Particle->GetPos(), (*m_Triangle)))
			{
				Point3 point = m_Triangle->GetPoint(i);
				Edge edge = m_Triangle->GetEdge(i);
				Point3 result;
	...
	{
		//Check plane
		CollisionPlane plane = m_Triangle->GetPlane();	
		Point3 result;
	...
	m_Triangle->SetClosestFeature(closestI, closestPoint);
