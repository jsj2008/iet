 ...
 *
 * This code was placed in the public domain by Stefan Gustavson
 * in January 2011.
 */
...
/*
 * 2D classic Perlin noise using an array instead of the permutation texture.
 * Tested 2010-08-15, with terrible results.
 * This is hideously slow without the integer operations of GLSL 1.30 and above.
 * OpenGL 2.1 just won't cut it here.
 */
float noise_new(const in vec2 P)
{
  ...
  const vec2 grad2[] = vec2[](vec2(2,1), vec2(2,-1), vec2(-2,1), vec2(-2,-1),
                      vec2(1,2), vec2(1,-2), vec2(-1,2), vec2(-1,-2));

  // Noise contribution from lower left corner
  vec2 grad00 = grad2[int(mod(perm[ int(mod(perm[ int(mod(Pi.x, 32))] + Pi.y, 32))], 8))].xy;
  float n00 = dot(grad00, Pf);
...
/*
 * 3D classic Perlin noise using an array instead of the permutation texture.
 * Tested with terrible results, see 2D version above for comments.
 * On a GeForce GTX260, this is 100 times slower than noise(vec3) below!
 */
float noise_new(const in vec3 P)
{
  ivec3 Pi = ivec3(floor(P)); // Integer part for permutation
  vec3 Pf = fract(P); // Fractional part for interpolation

  const int perm[] = int[32](0,8,2,3,23,22,13,28,30,19,1,6,26,10,9,15,14,24,31,27,16,21,20,5,17,29,18,11,12,4,7,25);
  const vec3 grad3[] = vec3[](vec3(0,1,1), vec3(0,1,-1), vec3(0,-1,1), vec3(0,-1,-1),
                      vec3(1,0,1), vec3(1,0,-1), vec3(-1,0,1), vec3(-1,0,-1),
                      vec3(1,1,0), vec3(1,-1,0), vec3(-1,1,0),vec3(-1,-1,0), // 12 cube edges
                      vec3(1,0,-1), vec3(-1,0,-1), vec3(0,-1,1), vec3(0,1,1)); // 4 repeats

  // Noise contribution from corner Pf=(0,0,0)
  vec3 grad000 = grad3[int(mod(perm[ int(mod(perm[ int(mod(perm[ int(mod(Pi.x, 32))] + Pi.y, 32))] + Pi.z, 32))], 16))];
  float n000 = dot(grad000, Pf);

  // Noise contribution from corner Pf=(1,0,0)
  vec3 grad100 = grad3[int(mod(perm[ int(mod(perm[ int(mod(perm[ int(mod(Pi.x+1, 32))] + Pi.y, 32))] + Pi.z, 32))], 16))];
  float n100 = dot(grad100, Pf - vec3(1.0, 0.0, 0.0));

  // Noise contribution from corner Pf=(0,1,0)
  vec3 grad010 = grad3[int(mod(perm[ int(mod(perm[ int(mod(perm[ int(mod(Pi.x, 32))] + Pi.y+1, 32))] + Pi.z, 32))], 16))];
  float n010 = dot(grad010, Pf - vec3(0.0, 1.0, 0.0));

  // Noise contribution from corner Pf=(1,1,0)
  vec3 grad110 = grad3[int(mod(perm[ int(mod(perm[ int(mod(perm[ int(mod(Pi.x+1, 32))] + Pi.y+1, 32))] + Pi.z, 32))], 16))];
  float n110 = dot(grad110, Pf - vec3(1.0, 1.0, 0.0));

  // Noise contribution from corner Pf=(0,0,1)
  vec3 grad001 = grad3[int(mod(perm[ int(mod(perm[ int(mod(perm[ int(mod(Pi.x, 32))] + Pi.y, 32))] + Pi.z+1, 32))], 16))];
  float n001 = dot(grad001, Pf - vec3(0.0, 0.0, 1.0));

  // Noise contribution from corner Pf=(1,0,1)
  vec3 grad101 = grad3[int(mod(perm[ int(mod(perm[ int(mod(perm[ int(mod(Pi.x+1, 32))] + Pi.y, 32))] + Pi.z+1, 32))], 16))];
  float n101 = dot(grad101, Pf - vec3(1.0, 0.0, 1.0));

  // Noise contribution from corner Pf=(0,1,1)
  vec3 grad011 = grad3[int(mod(perm[ int(mod(perm[ int(mod(perm[ int(mod(Pi.x, 32))] + Pi.y+1, 32))] + Pi.z+1, 32))], 16))];
  float n011 = dot(grad011, Pf - vec3(0.0, 1.0, 1.0));

  // Noise contribution from corner Pf=(1,1,1)
  vec3 grad111 = grad3[int(mod(perm[ int(mod(perm[ int(mod(perm[ int(mod(Pi.x+1, 32))] + Pi.y+1, 32))] + Pi.z+1, 32))], 16))];
  float n111 = dot(grad111, Pf - vec3(1.0, 1.0, 1.0));

  // Blend contributions along z
  vec4 n_z = mix(vec4(n000, n100, n010, n110), vec4(n001, n101, n011, n111), fade(Pf.z));

  // Blend contributions along y
  vec2 n_yz = mix(n_z.xy, n_z.zw, fade(Pf.y));

  // Blend contributions along x
  float n_xyz = mix(n_yz.x, n_yz.y, fade(Pf.x));
